...
document.querySelectorAll('.audit-item').forEach(item => {
  ...
  const metaEl = cloned.querySelector('.audit-meta'); // Korrigerat från .meta till .audit-meta
...
// Aktivera markörer med fullt a11y-stöd för skärmläsare och tangentbordsnavigering
document.querySelectorAll('.marker, .slutsvar-marker').forEach(marker => {
  if (!marker.hasAttribute('tabindex')) marker.setAttribute('tabindex', '0');
  ...
  marker.addEventListener('click', toggleTip);
...
// Stäng vid klick utanför marker
...
// Dölj tooltip momentant vid skroll och resize för att undvika flytande artefakter
